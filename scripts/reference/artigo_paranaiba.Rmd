...
# load(file = "/home/heron/prjcts/... .RData")
...
FINAL_TBL_prnb <- readxl::read_xlsx(path = "/home/heron/prjcts/paranaiba/EM118_Paranaiba/results/ecomol-EM118_Paranaiba-complete_analysis_results-2023-01-16.xlsx",
                                           col_names = TRUE,
                                    .name_repair = "minimal",
                                           guess_max = 200) %>% 
...
tbl_curadoria <- read.csv(file = "/home/heron/prjcts/paranaiba/data/ecomol-EM118_Paranaiba-CURADO-ASVs_Vs_Samples_12abr24.csv",
                          header = TRUE,
                          sep = ",",
                          check.names = F) %>% 
  as_tibble() %>% 
...
# tbl_curadoria <- readxl::read_xlsx(path = "/home/heron/prjcts/paranaiba/data/Curadoria_seq_Parnaiba-mar24.xlsx",
#                                    col_names = TRUE,
#                                    sheet = "Curadoria",
#                                    guess_max = 200) %>% 
...
cur_Daniel <- readxl::read_xlsx(path = "/home/heron/prjcts/paranaiba/data/Paranaiba-Complete_analysis_results-2024-04-12_tabela_paper.xlsx",
                                              sheet = "IDs X Samples",
                                           col_names = TRUE,
                                    .name_repair = "minimal",
                                           guess_max = 200) 
...
spec1 <- vegan::specaccum(comm = all_IDs_NMDS_df_jc[,5:ncol(all_IDs_NMDS_df_jc)])
spec2 <- vegan::specaccum(comm = all_IDs_NMDS_df_jc[,5:ncol(all_IDs_NMDS_df_jc)],
          method = "random")
...
#Collector curve per sample  ---- # nao funcionou
...
#rarefaction curve # nao funcionou
...
raremax <- min(rowSums(all_IDs_NMDS_df_jc[,5:ncol(all_IDs_NMDS_df_jc)])) # = 340; 
...
# figura de OTus e ASVs por amostras #nao rodou
...
# All markers ----
marker_IDs <- FINAL_TBL_prnb_cur_clean %>% 
  filter(`Kingdom (BLASTn)` %in% c("Metazoa")) %>%
...
FINAL_TBL_prnb_cur_clean %>% 
  ...
  geom_jitter(width = 0.5 ,
              height = 0.5 )+
  scale_color_manual(values = rev(viridis::viridis(n = length(unique(FINAL_TBL_prnb_cur_clean$`Phylum (BLASTn)`)))))
...
FINAL_TBL_prnb_cur_clean$agrupador %>% unique()
...
curated_smp_abd_ID$`Family (BLASTn)`[curated_smp_abd_ID$`Curated ID` %in% c("Apareiodon sp.1","Apareiodon sp.2")] <- "Parodontidae"


#create new tax table from BLASTn identifications
blast_tax_table_h <-  curated_smp_abd_ID %>%
...
# metadata_tbl_h <- FINAL_TBL %>% 
metadata_tbl_h <- curated_smp_abd_ID %>% 
...
names(FINAL_ID_table_h)
str(FINAL_ID_table_h)

## create phyloseq object ----
FINAL_PS_h <- phyloseq::phyloseq(otu_table(FINAL_ID_table_h,taxa_are_rows = T),
                               sample_names(metadata_tbl_h),
                               tax_table(blast_tax_table_h))
# 
# 
# phylo_rich_plot <- plot_richness(physeq = FINAL_PS_h, 
#                                  x="Metadata9",
#                                  shape = "Metadata9",
#                                  color = "Metadata9", 
...
#      dpi = 300)
 ...
 # estimate_richness(FINAL_PS_h)
  div_est_phylo_h <-  estimate_richness(FINAL_PS_h,
                                      split = T, 
...
#gr´afico de diversidade por altimetria ----
 ...
 # extraindo tabela por ponto amostral par ao manuscrito
elevation_plot <- div_est_phylo_h %>% 
...
vegan::adonis2(t(FINAL_ID_table_h) ~ Estado + Status, data = metadata_tbl_h, by = NULL)
vegan::adonis2(t(FINAL_ID_table_h) ~  Status , data = metadata_tbl_h)
vegan::adonis2(t(FINAL_ID_table_h) ~ Estado * Status, data = metadata_tbl_h)


colnames(FINAL_ID_table_h)
rownames(t(FINAL_ID_table_h))
...
dists <- vegdist(t(FINAL_ID_table_h),method = "bray")
dists <- vegdist(t(FINAL_ID_table_h),method = "jaccard")

perm <- betadisper(d = dists,
                   group = interaction(metadata_tbl_h$Estado,metadata_tbl_h$Status,metadata_tbl_h$`Environment (material)`))
...
head(FINAL_ID_table_h)
head(blast_tax_table_h)
...
phyloseq::tax_table(blast_tax_table_h)
phyloseq::phy_tree() 
...
#create new tax table from BLASTn identifications
blast_tax_table_h <- curated_smp_abd_ID %>% select(c("Curated ID",
                        "Genus (BLASTn)",
                        "Subfamily (BLASTn)",
                        "Family (BLASTn)",
                        "Suborder (BLASTn)",
...
colnames(blast_tax_table_h) <- colnames(blast_tax_table_h) %>% str_replace(" ","_") %>% str_remove_all(pattern = "\\(|\\)")
colnames(blast_tax_table_h) <- colnames(blast_tax_table_h) %>% str_remove_all(pattern = " \\(BLASTn\\)")



all_ps@tax_table <- blast_tax_table_h #N'ao funciona
...
tax_table(blast_tax_table_h) %>% View()
