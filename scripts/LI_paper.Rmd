...
# Tile plots 
{
  ...
  # Tile plots 
  {
    ...
    tile_mce_vs_sterivex <- grouped_filt_tbl %>%
      # filter(RRA >= 0.01) %>%
      mutate(`Curated ID` = factor(`Curated ID`, levels = rev(spp_levels))) %>%
      mutate(Site = factor(Site)) %>% 
      mutate(Order = factor(Order)) %>%
    ...
    tile_sbg_full_vs_low <- grouped_filt_tbl %>%
      # filter(RRA >= 0.01) %>%
      filter(Filter %in% c("MCE")) %>%
      mutate(`Curated ID` = factor(`Curated ID`, levels = rev(spp_Levels))) %>% 
      mutate(Site = factor(Site)) %>%
...
ggplot(aes(x = Individuals,
    ...
    xlab(label = "Number of ASVs") +
    ggtitle("Collector's curve MCE and Sterivex samples")
...
# All samples (Richness vs ASVs)
{
  ...
  collector_plot_all_ASVs <- 
    ...
    ylab(label = "Species richness") +
    xlab(label = "Number of ASVs") +
    ggtitle("Collector's curve MCE and Sterivex samples (ASVs)") 
