---
title: "Sterivex versus MCE Analyses for APQ-02247-23 FAPEMIG  project"
format: html
theme: cerulean
code-block-bg: true
...
-   Create custom functions
    ...
    ```{r}
    # Rarefy and calculate diversity indexes community data
    
    calc_rarefied_richness <- function(count_matrix, depth, index = "shannon", n_iter = 100, seed = 123) {
      
      # 1. Set seed for reproducibility
      set.seed(seed)
      
      # 2. Prepare matrix to store results (Rows = Samples, Cols = Iterations)
      result_matrix <- matrix(NA, nrow = nrow(count_matrix), ncol = n_iter)
      rownames(result_matrix) <- rownames(count_matrix)
      
      # 3. Iteration Loop
      cat(paste0("Calculating ", index, " with ", n_iter, " rarefaction iterations...\n"))
      
      for (i in 1:n_iter) {
    # Suppress vegan warnings regarding "counts 1" to keep console clean
    suppressWarnings({
      # a) Random subsampling (Rarefaction)
      temp_table <- rrarefy(count_matrix, sample = depth)
      
      # b) Calculate the chosen index
      if (index %in% c("shannon", "simpson", "invsimpson")) {
        result_matrix[, i] <- diversity(temp_table, index = index)
      } else if (index == "richness") {
        # For richness (Observed), we use specnumber
        result_matrix[, i] <- specnumber(temp_table)
      } else {
        stop("Index not supported. Please use: 'shannon', 'simpson', 'invsimpson', or 'richness'.")
      }
    })
      }
      
      # 4. Consolidate results (Mean and Standard Deviation)
      final_df <- tibble(
    Sample = rownames(count_matrix),
    Index_Type = index,
    Mean_Value = rowMeans(result_matrix),   # The robust mean value
    SD_Value = apply(result_matrix, 1, sd), # The standard deviation across iterations
    Depth = depth,
    Iterations = n_iter
      )
...
-   **Table S5. Post-curation and filtering results for retained species.** This table presents the total read abundance per species across all samples (Total abundance), absolute number of reads per species at each sampling point and campaign (Reads), number of ASVs (ASVs) and OTUs (OTUs) assigned to each species, and Relative Read Abundance (RRA). This table reflects the final dataset used for statistical and ecological analyses after integrative curation (see Methods).
    ...
    grouped_sampled_filt_tbl_stx_vs_mce <- 
      curated_full_tbl_stx_vs_mce %>% 
      filter(`Class (BLASTn)` %in% "Actinopteri",
             !`Curated ID` %in% c("Salmo salar")) %>% 
...
-   **Table S6. Observed alpha-diversity values per sampling site and mean diversity per water-level period.** Observed alpha-diversity values per sampling site and mean diversity per water-level period. Alpha-diversity (Observed index) across sampling sites, showing raw values for each location and mean diversity per water-level period. The Observed index represents the total number of distinct species detected in each sample.
    ...
    grouped_filt_site_stx_vs_mce <- 
      grouped_filt_tbl_stx_vs_mce %>%
      # group_by(Level, Site) %>% 
      group_by(Filter, Site) %>%
      mutate("Diversity" = length(unique(`Curated ID`))) %>%
      ungroup() %>% 
      # select(Level, Site, Diversity) %>% 
      select(Filter, Site, Diversity) %>% 
      unique() %>% 
      # group_by(Level) %>%
      group_by(Filter) %>%
      mutate("Mean" = mean(Diversity),
             "SD" = sd(Diversity))
...
-   **Table 2. Taxa detected by eDNA metabarcoding.** Taxa detection after integrative curation, showing taxonomic assignment and corresponding absolute read abundance, ASVs, and OTUs.
    ...
    id_level <- grouped_filt_tbl_stx_vs_mce %>%
...
-   **Figure 2A.** **Taxa composition across sampling sites and water levels.** The tile plot displays the relative read abundance (RRA) of each taxa at different sites (P1–P4) and water levels (High and Low). (Char = Characiformes, Cich = Cichliformes, Sil = Siluriformes and Cypr = Cyprinodontiformes).
    ...
    order_levels_stx_vs_mce <- 
      grouped_filt_tbl_stx_vs_mce$Order %>% 
      sort() %>% 
      unique()

    spp_levels_stx_vs_mce <- 
      grouped_filt_tbl_stx_vs_mce$`Curated ID` %>% 
      sort() %>% 
      unique()
    ...
    tile_all_stx_vs_mce <- 
      grouped_filt_tbl_stx_vs_mce %>%
      mutate(`Curated ID` = factor(`Curated ID`, 
                               levels = rev(spp_levels_stx_vs_mce)),
         Site = factor(Site),
    ...
    # Exporting plot for the paper
    ggsave(plot = tile_all_stx_vs_mce,
...
-   Prepare data and create a matrix object to rarefy the community data and calculate alpha diversity indices.
    ...
    otu_tbl_abd <- 
      ...
      pivot_wider(id_cols = Sample,
                  ...
                  names_sort = TRUE,
                  values_fill = 0) %>%
      as.data.frame() %>% # Tibbles dont accept rownames
      column_to_rownames(var = "Sample") %>%
      dplyr::filter(rowSums(.)!= 0) %>% # Remove rows (samples) without species
      dplyr::select(which(!colSums(., na.rm = TRUE) %in% 0)) # Remove colunms (species) not find in any samples

    # As matrix for Vegan
    otu_tbl_abd_mtx <- as.matrix(otu_tbl_abd) 
    ```

-   Rarefy the community data and estimate alpha diversity indices

    ```{r}
    # Which sample is the smallest? 
    otu_tbl_abd %>% rowSums() %>% sort() #P3_High_2022 = 31753
    
    # Rarefy the community data
    
    rare_tbl_abd <- rrarefy(otu_tbl_abd_mtx,
                            sample = 31753)
    ...
    # Observed
    obs_rich_tbl <- 
      calc_rarefied_richness(count_matrix = otu_tbl_abd_mtx,
                             depth = 31753,
                             index = "richness",
                             seed = 123,
                             n_iter = 1000) %>% 
      select(Sample, Mean_Value) %>% 
      rename(Mean_Value = "Observed")
    ...
    simpson_rich_tbl <- 
      calc_rarefied_richness(count_matrix = otu_tbl_abd_mtx,
                             ...
                             n_iter = 1000) %>% 
      select(Sample, Mean_Value) %>% 
      rename(Mean_Value = "Simpson")
    
    # Merging diversity indexes in one tibble
    rare_rich_tbl <- obs_rich_tbl %>%
      left_join(by = "Sample", shannon_rich_tbl) %>% 
      left_join(by = "Sample", simpson_rich_tbl)
    
    ```

-   Alpha diversity indices without rarefying
...
wider_filt_tbl_redux <- grouped_filt_tbl_stx_vs_mce %>%
...
FINAL_tbl_IDs_rare <- wider_filt_tbl_redux %>%
  select(c("New_sample", 
           "Curated ID", 
           "Year", 
           "Level", 
           "Site", 
           "Filter", 
           "Reads")) %>% # Use absolute reads abundance instead of relative reads abundance
...
all_IDs_PCoA_df_rare <- FINAL_tbl_IDs_rare %>%
  mutate("Sample number" = row_number()) %>%
  relocate("Sample number") %>%
  as.data.frame()

row.names(all_IDs_PCoA_df_rare) <- 
  all_IDs_PCoA_df_rare$`Sample number`

# Clean species names to avoid plotting issues
colnames(all_IDs_PCoA_df_rare)[8:ncol(all_IDs_PCoA_df_rare)] <- 
  colnames(all_IDs_PCoA_df_rare)[8:ncol(all_IDs_PCoA_df_rare)] %>%
...
```{r}
# Filter data 
MCE_all_IDs_PCoA_df <- all_IDs_PCoA_df_rare %>% 
  select(where(~ any(. != 0)))

# Rarefy community data and create distance matrix 
pcOa_dist <- avgdist(x = MCE_all_IDs_PCoA_df[,7:ncol(MCE_all_IDs_PCoA_df)],
                     sample = min(colSums(MCE_all_IDs_PCoA_df[,8:ncol(MCE_all_IDs_PCoA_df)])), # Min number of reads
                     dmethod = "jaccard",
                     iterations = 1000, # Number of resamplings
                     binary = TRUE) # Transforms counts to presence/absence to calculate qualitative 
                                    # beta-diversity rather than quantitative structural differences.
# Run PCoA
...
cent <- aggregate(cbind(PCoA1, PCoA2) ~ Filter, 
                  data = site.scrs, 
                  FUN = "mean")

# Calculate ellipses for plotting
ord <- ordiellipse(ord = pcOa,
                   groups = MCE_all_IDs_PCoA_df$Filter,
                   display = "sites",
                   kind = "ehull", 
                   conf = 0.95, 
                   label = TRUE)
...
for(g in levels(as.factor(MCE_all_IDs_PCoA_df$Filter))){
  df_ell <- rbind(df_ell,cbind(as.data.frame(with(site.scrs[site.scrs$Filter==g,],vegan:::veganCovEllipse(
