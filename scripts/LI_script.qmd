...
-   Analysis of removed data
    ...
    filt_stats_IDs_all <- 
      ...
      mutate(Reason = case_when(
                `Obs. Curadoria` == "Possible contamination" & 
                  `Primer expected length` == "out of range" ~ 
                  "Possible contamination, out of range",
                `Obs. Curadoria` == "Possible contamination" ~ 
                  "Possible contamination",
                `Primer expected length` == "out of range" ~ 
                  "out of range")) %>% 
      reframe("Reads_t" = sum(Abundance),
              "Class" = paste(unique(`Class (BLASTn)`,
                                       collapse = ", ")),
              "Samples" = paste(unique(Sample), 
                                  collapse = ", ")) %>%
      ungroup() %>%
      mutate("%_Total" = round(Reads_t / 
                               sum(Reads_t) * 100,
                               digits = 4)) %>%
      select(`Curated ID`,
             `Final ID (BLASTn)`,
             ...
             Samples,
             Reason) %>% 
      arrange(desc(Reads_t))

    # Infos about fish IDs filtered from dataset

    filt_stats_fish <- 
      filt_asvs %>%
      filter(! is.na(`Curated ID`),
             `Class (BLASTn)` == "Actinopteri") %>% 
      group_by(`Curated ID`, `Final ID (BLASTn)`) %>%
      reframe("Reads_t" = sum(Abundance),
              "Class" = paste(unique(`Class (BLASTn)`,
                                       collapse = ", ")),
    ...
    filt_stats_IDs_full <- 
      ...
      group_by(`Curated ID`, `Final ID (BLASTn)`) %>%
      mutate("Reads_t" = sum(Reads_s),
             "Abundance_t" = sum(Abundance_s)) %>%
      ungroup() %>%         
      mutate("% Total" = Reads_t / Abundance_t * 100,
