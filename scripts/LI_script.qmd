...
```{r}
raw_resume <- raw_results_full_tbl %>% 
  group_by(Sample) %>%
...
seq_read_counts <- read_excel("/home/gabriel/projetos/old/peixes-eDNA/analises/dez_23/runs_2_4_5_EM156/results/LI-reads_and_seqs_counts2024-01-08.xlsx") %>%
  tibble() %>%
  # only MCE samples
...
grouped_filt_site <- grouped_filt_tbl %>%
  group_by(Level, Site) %>% 
  mutate(
...
pre_filt_reads <- curated_full_tbl %>% group_by(Sample) 
...
class_Level <- grouped_filt_tbl %>%
  reframe(
    ...
    "Species" = length(unique(`Curated ID`[grepl("^[A-Za-z]+\\s[A-Za-z]+$", `Curated ID`) & 
                                               !grepl("sp\\.", `Curated ID`)])), # IDs at species level
    "Nspecies" = length(unique(`Curated ID`)) - Species) # IDs at other taxonomic levels
...
# Exporting table for the paper
write.csv(order_level,paste0(tbl_path, "/order_level_tbl", "-",
                             Sys.Date(), ".csv", sep = ""))
```
...
-   This tile plot visualizes the Relative Read Abundance (RRA) of each taxon across different sites and water levels, faceted by taxonomic order.
...
-   Prepare data and create a phyloseq object to calculate alpha diversity indices.
    ...
    phyloseq_tbl <- phyloseq(otu_table(otu_tbl, 
                                   taxa_are_rows = TRUE),
...
-   Estimate and plot species richness (Observed index) as a boxplot, comparing high and low water levels.
    ...
    # Estimating richness ----
    rich_tbl <- 
      estimate_richness(phyloseq_tbl,
                        split = TRUE,
                        measures = c("Observed",
                                     "Chao1",
                                     "Shannon",
...
```{r}
# Executing PCoA for High vs. Low ----
...
combined_accum <- bind_rows(accum_mce_df, accum_low_df, accum_full_df) %>%
  mutate(Level = factor(Level, levels = c("All", 
                                          "Low", 
                                          "High")))
collector_plot_all <- combined_accum %>%
  ggplot(aes(x = Sites,
             y = Richness,
             color = Level,
             fill = Level )) +
