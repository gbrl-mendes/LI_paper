...
curated_ids_tbl <- # Curated identifications df
  curated_ids_tbl %>%
  select(
    "ASV header", "ASV (Sequence)", "Curated ID", "Final ID (BLASTn)", 
    "BLASTn pseudo-score", "Class (BLASTn)", "Curated Order (BLASTn)"
  ) %>%
  unique()
```
...
raw_results_tbl <- pre_raw_results_tbl %>%
  ...
  select(
    ...
    "BLASTn pseudo-score", "Class (BLASTn)", "Curated Order (BLASTn)",
    "Sample", "Unique_File_name", "OTU", "Abundance", "Obs. Curadoria",
    "Read origin", "Primer expected length", "ASV Size (pb)"
  )
```
...
blast_tax_less <- blast_tax[2:85] %>% 
  ...
  mutate("Genus (BLASTn)" = str_split_fixed(string = .$`Curated ID`, pattern = " ", n = 2)[,1]) %>% 
  select("ASV (Sequence)", "Family (BLASTn)", "Genus (BLASTn)")

# Join the taxonomy to the main table
raw_results_tbl <- raw_results_tbl %>%
  left_join(blast_tax_less, by = "ASV (Sequence)") %>% 
  select(
    "ASV header", "ASV (Sequence)", "Curated ID", "Final ID (BLASTn)", 
    "BLASTn pseudo-score", "Class (BLASTn)", "Family (BLASTn)", 
    "Genus (BLASTn)", "Curated Order (BLASTn)", "Sample", 
    "Unique_File_name", "OTU", "Abundance", "Obs. Curadoria", 
    "Read origin", "Primer expected length", "ASV Size (pb)"
  ) %>% 
  unique() %>% 
...
### Manual corrections and filtering

This section covers manual corrections based on expert knowledge and the application of filters to remove low-quality or irrelevant data.

-   Correcting taxonomic identifications after curation

```{r}
# Update families for specific genera (february 10th 2025 correction)
raw_results_tbl <- raw_results_tbl %>% 
  mutate(`Family (BLASTn)` = 
           ifelse(`Genus (BLASTn)` %in% c("Psalidodon", "Moenkhausia", "Astyanax", "Hemigrammus", "Oligosarcus"), 
                  "Acestrorhamphidae", `Family (BLASTn)`)) %>% 
  mutate(`Family (BLASTn)` = ifelse(`Genus (BLASTn)` %in% "Piabina",
                                    "Stevardiidae", `Family (BLASTn)`))

# Group Hoplias species (february 25th 2025 correction)
raw_results_tbl <- raw_results_tbl %>%
  mutate(`Curated ID` =
...
-   Preliminary data filtering
    ...
    We remove reads outside the expected size range, "NA" identifications, potential contaminants from birds and mammals with low scores, and data from negative controls.
    ...
    pre_filt_tbl <-
      ...
      filter(!(Sample %in% c("EM113_NEGPCR1", "EM135c4c5_NEGPCR1", "EM135c4c5_NEGPCR2", 
                             "EM149_NEGPCR1", "EM149_NEGPCR2", "EM156_NegPCR1", 
                             "S724_NEGPCR1", "br_jan_22", "br_nov21"))) %>% 
...
<!-- -->
```
-   Joining filtered data with metadata

    ```{r}
    curated_full_tbl <- left_join(pre_filt_tbl, metadata_tbl, by = "Sample") %>%
      select(
        "ASV header", "ASV (Sequence)", "Curated ID", "Final ID (BLASTn)", 
        "BLASTn pseudo-score", "Class (BLASTn)", "Family (BLASTn)", 
        "Genus (BLASTn)", "Order (BLASTn)", "Sample", "Unique_File_name", 
        "OTU", "Abundance", "Read origin", "Primer expected length", 
        "ASV Size (pb)", "Run", "Filter", "Site", "Expedition", "Year", "Level"
      ) %>% 
...
-   Verifying data loss
    ...
    raw_n_meta_tbl <- left_join(raw_results_tbl, metadata_tbl, by = "Sample") %>%
      filter(Filter %in% "MCE")
